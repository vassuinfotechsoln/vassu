...
const VOICE_OPTIONS = [
  { value: "alloy", label: "Alloy", gender: "Neutral", description: "Clear & professional" },
  { value: "echo", label: "Echo", gender: "Male", description: "Warm & conversational" },
  { value: "fable", label: "Fable", gender: "Male", description: "British accent" },
  { value: "onyx", label: "Onyx", gender: "Male", description: "Deep & authoritative" },
  { value: "nova", label: "Nova", gender: "Female", description: "Friendly & bright" },
  { value: "shimmer", label: "Shimmer", gender: "Female", description: "Soft & elegant" },
];
...
export default function AgentEditor({ agent, onSave, onCancel }) {
  ...
  const tempLabel =
    formData.temperature <= 0.3 ? "Focused (Precise answers)" :
      formData.temperature <= 0.6 ? "Balanced" :
        formData.temperature <= 0.8 ? "Creative" : "Very Creative (Experimental)";
  ...
  return (
    <div className="bg-[rgb(var(--color-surface))] border border-[rgb(var(--color-border))] rounded-[2.5rem] overflow-hidden shadow-2xl animate-in zoom-in-95 duration-500">
          ...
          <div className="flex items-center gap-6">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                <p className="text-white/80 text-sm font-medium">Professional AI Output Configuration</p>
              </div>
            </div>
          </div>
          <div className="flex items-center gap-3 bg-white/10 px-5 py-3 rounded-2xl backdrop-blur-sm border border-white/10">
            <Phone className="w-5 h-5 text-emerald-400" />
            <span className="text-white text-sm font-bold">Encrypted Twilio Integration</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex p-2 gap-2 border-b border-[rgb(var(--color-border))] bg-[rgb(var(--color-background))/0.5]">
        {[
          { id: "identity", icon: <User className="w-4 h-4" />, label: "Identity & Logic" },
          { id: "voice", icon: <Volume2 className="w-4 h-4" />, label: "Voice & Language" },
          { id: "behavior", icon: <Sliders className="w-4 h-4" />, label: "Creative Control" },
        ].map((tab) => (
          <button
            key={tab.id}
            type="button"
            onClick={() => setActiveSection(tab.id)}
            className={`flex-1 flex items-center justify-center gap-2 py-4 rounded-2xl text-sm font-bold transition-all duration-300 ${activeSection === tab.id
      ...
      <form onSubmit={handleSubmit} className="p-10 space-y-10">
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
            <div>
              ...
              <input
                ...
                className={cn(inputClass, errors.name && "border-rose-500 ring-rose-500/10")}
            ...
            <div>
              <label className={labelClass}>Linguistic Blueprints (Prompt Templates)</label>
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                <label className={labelClass}>Neural Instructions (System Prompt)</label>
                <div className="flex items-center gap-2 px-2 py-1 rounded-lg bg-indigo-50 dark:bg-indigo-900/30">
                  <span className="text-[10px] font-black text-indigo-600 uppercase">Tokens: {Math.ceil(formData.prompt.length / 4)}</span>
                </div>
              </div>
              <textarea
                ...
                rows={8}
                className={cn(inputClass, "resize-none font-mono text-xs leading-relaxed", errors.prompt && "border-rose-500 ring-rose-500/10")}
                placeholder="Enter the operating system instructions for this agent..."
              />
              {errors.prompt && <p className="text-rose-500 text-xs font-bold mt-2 ml-1">{errors.prompt}</p>}
            </div>
          </div>
        )}
        ...
        {activeSection === "voice" && (
          <div className="space-y-10 animate-in fade-in slide-in-from-bottom-2 duration-500">
            <div>
              <label className={labelClass}>Vocal Resonance (Acoustic Output)</label>
                ...
                {VOICE_OPTIONS.map((v) => (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div className={cn("w-12 h-12 rounded-xl flex items-center justify-center transition-colors", formData.voice === v.value ? "bg-indigo-600" : "bg-slate-200 dark:bg-slate-700")}>
                        <Mic className={cn("w-5 h-5", formData.voice === v.value ? "text-white" : "text-slate-400")} />
                      </div>
                      <div className="text-left">
                        <div className="font-black text-slate-900 dark:text-white">{v.label}</div>
                        <div className="text-[10px] font-bold text-slate-500 uppercase">{v.gender} · {v.description}</div>
                      </div>
                    </div>
                    {formData.voice === v.value && <div className="w-3 h-3 bg-indigo-500 rounded-full animate-pulse" />}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div>
              <label className={labelClass}>Cultural Dialect (Interaction Language)</label>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {LANGUAGE_OPTIONS.map((l) => (
                  ...
                  >
                    ...
                    <span className="font-black text-sm text-slate-900 dark:text-white uppercase tracking-tighter">{l.label}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="bg-indigo-600 rounded-3xl p-8 text-white relative overflow-hidden">
              ...
              <div className="relative z-10">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <h3 className="text-lg font-black uppercase tracking-tight">Cognitive Entropy</h3>
                </div>
                <div className="flex items-center justify-between mb-10">
                  <p className="text-indigo-100 text-sm font-medium italic">Adjust the variance of AI's neural connections</p>
                  <div className="text-4xl font-black">{formData.temperature.toFixed(1)}</div>
                </div>
                <input
                  type="range"
                  min="0"
            ...
            <div className="p-8 rounded-3xl border-2 border-slate-100 dark:border-slate-800 bg-[rgb(var(--color-background))]">
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center py-2 border-b border-slate-100 dark:border-slate-800/50">
                  ...
                  <span className="text-xs font-bold text-slate-700 dark:text-white">TURBO INFRASTRUCTURE (READY)</span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-slate-100 dark:border-slate-800/50">
                  <span className="text-[10px] font-black uppercase text-slate-400">Privacy Protocols</span>
                  <span className="text-xs font-bold text-slate-700 dark:text-white">ENCRYPTED AT REST</span>
                </div>
        ...
        <div className="flex items-center justify-between pt-10 border-t border-[rgb(var(--color-border))]">
          <Button type="button" variant="ghost" className="gap-2" onClick={onCancel}>
          ...
          <div className="flex items-center gap-4">
            {activeSection !== "behavior" && (
              <Button
                type="button"
                variant="outline"
                onClick={() => setActiveSection(activeSection === "identity" ? "voice" : "behavior")}
              >
                Continue Tuning
              </Button>
            )}
            ...
            >
              ...
              {isSaving ? "Synchronizing..." : agent ? "Update Neural Core" : "Initialize Agent"}
            </Button>
          </div>
        </div>
      </form>
